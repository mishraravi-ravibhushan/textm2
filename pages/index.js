...
const Home = () => {
  ...
  const lcaluser = JSON.parse(storedUser);
  const SIP_CONFIG = {
    realm: "43.244.140.123:5066", // SIP domain
    impi: lcaluser.SipID, // SIP username
    impu: `sip:${lcaluser.SipID}@143.244.140.123:5066`, // SIP URI
    password: lcaluser.SipID,
  ...
  const startSip = () => {
    const stack = new SIPml.Stack({
      ...
      display_name: lcaluser.userName,
      ...
      events_listener: {
        ...
        listener: (e) => {
          ...
          if (e.type === "i_new_call") {
            ...
            console.log(
              "line no 92",
              e.o_event.o_session.o_stack.identity.s_display_name
            );
            setIncomingFrom(
              e.o_event.o_session.o_stack.identity.s_display_name
            );
            ...
            console.log("incomming call session ", incomingSession);
            console.log("Call From: ", incomingSession.getRemoteFriendlyName());
            setTimeout(() => {
              var remoteName = incomingSession.getRemoteFriendlyName();
              console.log("Calling:", remoteName); // Often null unless peer sends display name
            }, 500);
          }
  ...
  const toggleMute = () => {
    ...
    try {
      if (isMuted) {
        var bMute = !session.bMute;
        session.mute("audio", bMute);
      } else {
        var bMute = !session.bMute;
        session.mute("audio", bMute);
      }
        ...
        <Modal show={true} onHide={() => setShowCallPopup(false)}>
          ...
          <Modal.Body>
            <p className="d-flex gap-3 justify-content-center">
              You have an incoming call from {incomingFrom}
            </p>
