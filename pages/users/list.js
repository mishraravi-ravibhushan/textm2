...
const List = (searchParams) => {
  ...
  const removeSiteId = async (siteID) => {
    try {
      if (isEdit) {
        let data = new FormData();

        data.append("mode", "DELETSITEMAP");
        data.append("siteID", siteID);
        data.append("userID", editedUser.userID);
        await axios.post(API_BASE_URL + "sitemap", data);
      }
    } catch (e) {}
  };
        ...
        <Row className="mb-4">
          <Col lg={12} md={12} xs={12}>
            <div className="d-flex justify-content-between align-items-center mb-3">
              <h3 className="mb-0 text-white">User List</h3>
              {loginUser?.roleName &&
                roleaddbtn.includes(loginUser.roleName.toLowerCase()) && (
                  ...
                  >
                    Add User <i className="fe fe-user-plus ms-2"></i>
                  </Button>
                )}
            </div>
          </Col>
                        ...
                        <div className="hover-top-in text-center">
                          ...
                          >
                            ...
                            <ButtonGroup>
                              ...
                              <Button style={{ background: "#fbb219" }}>
                                <Link
                                  href={{
                                    pathname: "/",
                                    query: {
